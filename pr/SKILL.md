...
1. Run bundled `bash scripts/pr-preflight` from the target worktree before review; publication runs its own preflight.
2. For a default or spent branch, carry only intended commits onto a fresh branch from the reported base.
3. Publish only to the current branch's writable remote. Do not switch to a parent/upstream repository or create/sync a fork unless explicitly asked.

## Review

1. Using the preflight base and head, review the prospective commits and complete `base...head` diff for scope and actionable issues.
2. Fix and report findings within the authorized scope and commit the fixes. Ask when resolution requires a material scope decision; report unresolved blockers without publishing.
...
3. For UI changes, follow [UI evidence](references/ui-evidence.md).
4. If the reviewed revisions change, repeat the affected review, checks, and UI captures before publication.
